...
const Header = (props) => {
  ...
  // Replace your useEffect for scroll handling with this:
  useEffect(() => {
    const headerElement = headerRef.current;

    if (!headerElement) return; // Early return if ref not set

    const handleScroll = () => {
      const scrollPosition = window.pageYOffset || document.documentElement.scrollTop;

      // Add a small delay to ensure ref is available
      requestAnimationFrame(() => {
        if (headerElement) { // Double-check element exists
